...
function ProductsPage() {
  ...
  async function handleAddToCart(productId) {
    // TODO en clase: conectar thunk real y refrescar carrito desde backend.
    await addCartItem(productId, 1);
    dispatch(addLocalCartItem({ productId, quantity: 1 }));
  }

  async function handleToggleWishlist(productId) {
    // TODO en clase: sincronizar respuesta real del backend con Redux.
    await toggleWishlist(productId);
    dispatch(toggleLocalWishlist(productId));
  }

  if (loading) {
    return <StatusMessage title="Cargando productos" description="Consultando catalogo..." />;
  }
    ...
    <main className={styles.page}>
      <section className={styles.hero}>
        ...
        <p className={styles.copy}>
          Ver producto, anadir al carrito, persistir en backend y actualizar el estado global.
        </p>
      </section>
